...
contract BCToken is Context, IERC20, Ownable {
    ...
    constructor(address _nftAddress, address payable routerAddress, address _usdtAddress, uint256 _presaleEndsIn) {
    ...
    function _getValues(uint256 tAmount, address _account) private view returns (GetValues memory) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rRewardFee) =
            _getRValues(tAmount, tRewardFee, tBurnFee, tTaxFee);
        GetValues memory newValue = GetValues(rAmount, rTransferAmount, rRewardFee, tTransferAmount, tRewardFee, tBurnFee, tTaxFee);
        return newValue;
    }
    ...
    function _getRValues(uint256 tAmount, uint256 tRewardFee, uint256 tBurnFee, uint256 tTaxFee) private view
        returns (
            uint256,
            uint256,
            uint256
